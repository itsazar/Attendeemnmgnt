...
export default function Home() {
  ...
  const [attendanceResult, setAttendanceResult] = useState<AttendanceSummary | null>(null);
  const [attendanceLoading, setAttendanceLoading] = useState(false);
  const [selectedAttendanceEvent, setSelectedAttendanceEvent] = useState<string>("");

  const [downloadKey, setDownloadKey] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [searchField, setSearchField] = useState<"email" | "company" | "city">("email");
  const [eventSort, setEventSort] = useState<"eventDate" | "attended" | "noShows">(
    "eventDate"
  );
  const [activeTab, setActiveTab] = useState<"dashboard" | "import" | "events" | "history" | "blocklist">("dashboard");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorData = { error: responseText || `Failed to load dashboard data (${response.status})` };
        }
        throw new Error(errorData.error || `Failed to load dashboard data (${response.status})`);
      }
      ...
      if (!selectedAttendanceEvent && data.eventHistory && data.eventHistory.length) {
  ...
  const handleImport = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Refresh overview in the background - don't fail the import if this fails
      try {
        await refreshOverview();
      } catch (refreshError) {
        console.warn("Failed to refresh overview after import:", refreshError);
        // Don't show this as an import error since the import succeeded
      }
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Unable to import participants";
      console.error("Import error:", error);
      setOverviewError(errorMessage);
      // Don't set importResult on error - let the user see the error message
    } finally {
  ...
  const handleAttendance = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error ?? "Attendance update failed");
      }
      const data = await response.json();
  ...
  const handleDownload = async (
    ...
    try {
      ...
      setOverviewError(error instanceof Error ? error.message : "Unable to download file");
    } finally {
      setDownloadKey(null);
    }
  };
  ...
  return (
    <div className="dashboard-layout">
        ...
        <section className="section">
          <div className="section-header">
            ...
            <span className="muted">
              {loadingOverview ? "Syncing latest data..." : "Live analytics across all events"}
            </span>
          </div>
          {summary ? (
            <>
                    ...
                    <div className="chart-bar-item">
                      ...
                      <div className="chart-bar">
                        <div
                          ...
                          style={{
                            width: `${
                              summary.totalAttended + summary.totalNoShows > 0
                                ? (summary.totalAttended / (summary.totalAttended + summary.totalNoShows)) * 100
                                : 0
                            }%`,
                          }}
                        />
                      ...
                      <div className="chart-bar">
                        <div
                          ...
                          style={{
                            width: `${
                              summary.totalAttended + summary.totalNoShows > 0
                                ? (summary.totalNoShows / (summary.totalAttended + summary.totalNoShows)) * 100
                                : 0
                            }%`,
                          }}
                        />
                ...
                <div className="chart-card">
                  ...
                  <div className="chart-circle-container">
                    <div className="chart-circle">
                      <svg viewBox="0 0 120 120" className="chart-circle-svg">
                        <circle
                          cx="60"
                          cy="60"
                          r="50"
                        ...
                        <circle
                          ...
                          r="50"
                          fill="none"
                          stroke={summary.noShowPercentage > 20 ? "#ef4444" : summary.noShowPercentage > 10 ? "#f59e0b" : "#10b981"}
                          strokeWidth="10"
                ...
                <div className="chart-card">
                  ...
                  <div className="chart-pie-container">
                    <div className="chart-pie-item">
                      <div className="chart-pie-color" style={{ background: "#10b981" }} />
                      <div className="chart-pie-info">
                        <span>Active</span>
                        <strong>{summary.totalParticipants - summary.totalBlocklisted}</strong>
                      </div>
                    </div>
                    <div className="chart-pie-item">
                      <div className="chart-pie-color" style={{ background: "#ef4444" }} />
                      <div className="chart-pie-info">
                        <span>Blocklisted</span>
                        <strong>{summary.totalBlocklisted}</strong>
                      </div>
                    ...
                    <div className="chart-pie-item">
                      <div className="chart-pie-color" style={{ background: "#3b82f6" }} />
                      <div className="chart-pie-info">
                        <span>Total</span>
                        <strong>{summary.totalParticipants}</strong>
                      </div>
      ...
      {activeTab === "import" && (
        <>
          <section className="section">
            <div className="section-header">
              ...
              <span className="muted">Upload the confirmed attendee spreadsheet</span>
            </div>
        <form onSubmit={handleImport}>
          <div className="form-grid">
            ...
            <label>
              ...
              <input type="file" name="confirmedFile" accept=".xlsx,.xls" required />
            </label>
          </div>
        ...
        {importResult && (
          <div className="message success" style={{ marginTop: "1rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.5rem", flexWrap: "wrap", gap: "0.5rem" }}>
              <div>
                <strong>Import summary:</strong> Imported {importResult.summary.totalImported}{" "}
                rows · {importResult.summary.newParticipants} new ·{" "}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const normal = (importResult.normalParticipants || []).map((p) => ({
                  ...
                  const noShows = (importResult.noShowParticipants || []).map((p) => ({
                    "Full Name": p.fullName,
                    Email: p.email,
                    Company: p.company ?? "",
                    City: p.city ?? "",
                  ...
                  const blocklisted = (importResult.blocklistedParticipants || []).map((p) => ({
                    "Full Name": p.fullName,
                    Email: p.email,
                    Company: p.company ?? "",
                    City: p.city ?? "",
                  ...
                  const allParticipants = [...normal, ...noShows, ...blocklisted];
                  ...
                  try {
                    const buffer = buildWorkbookFromRows("all_participants", allParticipants);
                    ...
                    alert("Failed to export participants. Check console for details.");
                  }
                }}
            ...
            <div style={{ marginTop: "0.75rem" }}>
              {importResult.normalParticipants && importResult.normalParticipants.length > 0 && (
                <div style={{ marginBottom: "0.75rem" }}>
                  <p><strong>Normal Participants ({importResult.normalParticipants.length}):</strong></p>
                  <ul>
                    {importResult.normalParticipants.map((participant) => (
              ...
              {importResult.noShowParticipants && importResult.noShowParticipants.length > 0 && (
                <div style={{ marginBottom: "0.75rem" }}>
                  <p><strong>Previous No-Shows ({importResult.noShowParticipants.length}):</strong></p>
                  <ul>
                    {importResult.noShowParticipants.map((participant) => (
                      <li key={participant.email}>
                        {participant.fullName} ({participant.email}) • Previous No-Show
                      </li>
                    ))}
                  </ul>
                </div>
              )}
              {importResult.blocklistedParticipants && importResult.blocklistedParticipants.length > 0 && (
                <div style={{ marginBottom: "0.75rem" }}>
                  <p><strong>Blocklisted ({importResult.blocklistedParticipants.length}):</strong></p>
                  <ul>
                    {importResult.blocklistedParticipants.map((participant) => (
                      <li key={participant.email}>
                        {participant.fullName} ({participant.email}) • Blocklisted
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <section className="section">
            <div className="section-header">
              ...
              <span className="muted">Mark attended, no-show, and blocklisted lists after the event</span>
            </div>
          ...
          <div className="form-grid">
            <label>
              ...
              <select
                ...
                onChange={(event) => setSelectedAttendanceEvent(event.target.value)}
                required
              >
            ...
            <label>
              Attended List
              <input type="file" name="attendedFile" accept=".xlsx,.xls" />
            </label>
            <label>
              No-Show List
              <input type="file" name="noShowFile" accept=".xlsx,.xls" />
            </label>
            <label>
              Blocklisted List
              <input type="file" name="blocklistedFile" accept=".xlsx,.xls" />
            </label>
          </div>
          <button type="submit" disabled={attendanceLoading}>
            {attendanceLoading ? "Recording attendance..." : "Update Attendance"}
          </button>
        </form>
        {attendanceResult && (
          <div className="message success" style={{ marginTop: "1rem" }}>
            ...
            {attendanceResult.summary.missingParticipants.length > 0 && (
            ...
            <tbody>
              {sortedEventHistory.map((event) => (
                <tr key={event.id}>
                  ...
                  <td>
                    <span className="status-pill attended">{event.attended}</span>
                  </td>
                  <td>
                    <span className="status-pill no-show">{event.noShows}</span>
                  </td>
                  <td>{event.flagged}</td>
                  <td>
                    <div className="actions">
      ...
      {activeTab === "blocklist" && (
        <section className="section">
          ...
          >
            {downloadKey === "blocklist" ? "Preparing..." : "Export Blocklist"}
          </button>
        </div>
        <div className="filters">
          <input
            ...
            <tbody>
              {filteredBlocklist.map((entry) => (
                <tr key={entry.id}>
                  ...
                  <td>{entry.firstNoShowAt ? formatDate(entry.firstNoShowAt) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
